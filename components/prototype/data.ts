...
export const onboardingSlides: OnboardingSlide[] = [
  {
    ...
    icon: HeartHandshake,
    title: "Добрый день!",
    text: "Рады приветствовать вас в ИИ-Ассистенте. Это чат-бот, которого разработали психологи для бережного сопровождения — более безопасного, чем разговор с ChatGPT или Claude.",
  },
  {
    kind: "compare",
    title: "Почему с ним себе не навредишь",
    subtitle: "Обычный ИИ-чат и наш ассистент ведут себя по-разному",
    cons: [
      "Соглашается почти со всем, чтобы понравиться",
      "Может поддержать вредную для вас идею",
      "Не знает клинических границ и протоколов",
    ],
    pros: [
      "Держит границы и не поддакивает во вред",
      "Опирается на протоколы клинических методик",
      "Понимает, когда нужна живая помощь",
    ],
    privacyNote:
      "Чат анонимен и конфиденциален. Данные хранятся в зашифрованном виде — только чтобы держать контекст вашего диалога, и не более. Потом они автоматически удаляются.",
  },
  {
    kind: "features",
    title: "Соберём ассистента под тебя",
    subtitle: "С помощью конструктора можно точечно начать работу именно с вашей ситуацией",
    features: [
      {
        icon: Scale,
        title: "Знает, где остановиться",
        text: "Бот чётко понимает границы: где стоит соглашаться, а где — прекратить поддакивать. Любая нейросеть рано или поздно начнёт давать ответ, который вы желаете получить.",
      },
      {
        icon: SlidersHorizontal,
        title: "Тема и метод",
        text: "Кроме темы разговора можно выбрать метод работы — по протоколам современных методик из клинических руководств.",
      },
      {
        icon: LifeBuoy,
        title: "Сопровождает, а не учит жизни",
        text: "Бот позволит моментально получить информацию по вашему состоянию и техники самопомощи, но не заменяет реальную психотерапию.",
      },
    ],
  },
];
